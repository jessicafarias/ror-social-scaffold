      ...
      list_item +=
        ...
        content_tag(:div, class: 'd-flex') do
          content_tag(:span, class: 'profile-link mr-2') do
            button_to 'See Profile', user_path(user), method: 'get', class: 'grey profile-link'
          end +
    ...
    if current_user.invitee?(user) && !user.friend?(current_user)
      content_tag(:div, class: 'd-flex') do
        content_tag(:span, class: 'profile-link') do
          button_to 'Accept', "/friendships/#{user.id}", method: 'put', class: 'profile-link'
        end +
          content_tag(:span, class: 'profile-link') do
      ...
      content_tag(:span, class: 'profile-link') do
        button_to 'Pending', class: 'mr-2 profile-link'
      end
    elsif !user.friend?(current_user)
      content_tag(:span, class: 'profile-link') do
        button_to 'Add Friend', "/adding/#{user.id}", method: 'get', class: 'mr-2 profile-link'
      end
    else
      content_tag(:span, class: 'profile-link') do
        button_to "Remove #{user.id}", "/friendships/#{user.id}", method: 'delete', class: 'profile-link'
      end
    end
  end
end
